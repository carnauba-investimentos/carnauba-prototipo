...
- **Cronograma Gantt interativo** — visualização em meses com barras de progresso por grupo e por item; grupos recolhíveis com drag-and-drop para reordenação
  - **Barras de meses passados esticadas (modo Físico)** — qualquer etapa cujo mês encerrou (passado) tem sua barra esticada da coluna original até o final do mês atual; as etapas seguintes do mesmo item são empurradas para uma segunda linha; item e grupo crescem verticalmente na mesma proporção; vale tanto para etapas em atraso quanto para etapas concluídas dentro de meses passados; o `GanttItemCard` e o `GanttGroupCard` ajustam largura e altura dinamicamente para acompanhar o conteúdo
  - **Rótulo de mês acima de cada barra** — cada `GanttItemBar` e `GanttGroupBar` exibe o mês de início no formato `"mai/26"` imediatamente acima da barra; a cor do rótulo espelha o estado da barra: cinza (futuro), azul (em dia), âmbar (em atraso / overrun financeiro); altura das linhas de item aumentada para acomodar o rótulo
